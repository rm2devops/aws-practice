# Creating a subnet inside a VPC. This module can be used to create either a private or public-facing subnet
...
# We are creating one or more subnets that we want to address as one, therefore we create a routing table and
...
resource "aws_route_table" "subnet" {
  ...
  tags {
    Name        = "${var.name}-${element(var.availability_zones, count.index)}-rt"
    Environment = "${var.environment}"
  }
}
